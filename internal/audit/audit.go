// Package audit 是账号服务的审计写入端：把「谁、在什么时候、对什么、做了什么、结果如何」
...
//  3. **只记登记过的写路由**：动作码注册表（Options.Actions）是唯一开关，新增写端点忘了登记
//     会被 handler 包的「写路由覆盖守卫」测试拦住。
package audit
...
//
// 为什么用 audit 独立 schema：它不是任何单个服务的领域数据（账号服务读得到 catalog/community/
// storage 的行是刻意的，见契约 §5）；为什么不建外键：账号删了审计还得在。
// pg_advisory_xact_lock(740205) 让四个服务首次启动时串行建表：一次 Exec 里的多条语句由
// lib/pq 作为隐式事务批处理发送，锁因此覆盖到建表结束，避免两条 CREATE TABLE 撞在
// pg_type 的唯一索引上（storage 的迁移器注释里记过这个坑，锁键与三个服务各自主用的
// 迁移锁 740202/740203/740204 区分开）。
//
// **为什么整段包在 to_regclass 守卫里**：PostgreSQL 的 `CREATE INDEX IF NOT EXISTS` 会**先做表
// 所有权检查、再看索引是否存在**（`CREATE TABLE IF NOT EXISTS` 不同，它只要求 schema 的 CREATE）。
// 四个服务启动都会执行这段 DDL，而表由部署时的 mf_audit_owner 预建 —— 不做守卫的话，非 owner 的
// 运行角色会在启动时拿到 `42501 must be owner of table audit_log`，服务直接起不来。
// 守卫让"表已存在"的实例上整段变成纯空转（契约 §6.1 有实测记录）。
const Schema = `
DO $audit_ddl$
BEGIN
...
// 凭据类型取值（契约 §1 的 credential_type 列）。
const (
...
// 供不建库的用例使用（与 store 的令牌签发器同一降级思路：不可用，但不静默假装成功）。
...
func (r *Recorder) loop() {
	...
	for j := range r.jobs {
		if j.entry != nil {
			if err := r.insert(*j.entry); err != nil {
				// 只有这一处会丢掉审计行，所以必须吵：丢行意味着"发生了没人知道的写操作"。
				slog.Error("账号服务：审计写入失败（该行已丢弃，业务不受影响）", "action", j.entry.Action,
...
func (r *Recorder) Record(e Entry) {
	...
	default:
		r.dropped.Add(1)
		slog.Error("账号服务：审计队列已满，丢弃一条审计行", "action", e.Action, "request_id", e.RequestID)
	}
}

// RecordSync 同步写入一条审计行（测试用，以及个别"必须与业务同生共死"的动作）。
...
// Close 排空队列并停掉后台 goroutine（测试收尾；服务进程退出时不必调用）。
...
var redactedExactKeys = map[string]bool{
	...
	"authorization": true, "cookie": true, "code_verifier": true, "code_challenge": true,
}

// redactedKeyParts 是整键丢弃的子串黑名单（宁可少记，不可泄漏）。
var redactedKeyParts = []string{"password", "secret", "token", "hash"}

// emailPattern 用于"值里顺手带了邮箱"的情形。
...
//
// 三层防线：键名黑名单 → 键名含 email → 值里出现邮箱形状的串。深度上限 6 层，
...
// MaskEmail 把邮箱遮罩成 j***@example.com：保留首字母与域名，去掉可识别的人名部分。
// 非邮箱形状的输入按原样截断返回——它不是通用脱敏器（那是 SanitizeChanges 的活）。
...
func truncate(s string, limit int) string {
	// 按 rune 边界截断，避免把多字节字符切成半个（用户名可能是中文）。
	runes := []rune(s)
	if len(runes) <= limit {
		return s
	}
...
// Options 是中间件参数。
type Options struct {
	// Recorder 为 nil 时中间件退化为空操作（测试与"没建库"的进程都能跑）。
	Recorder *Recorder
...
func Middleware(o Options) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		action, ok := o.Actions[c.Request.Method+" "+c.FullPath()]
		if !ok {
			c.Next()
			return
		}
		// request_id 先落：处理器与下游日志都要能用同一个 id 串起来。
		...
		// 身份与补充信息都在 c.Next() 之后读：处理器可能刚把 actor 设成"这次操作的对象所属者"
		// （登录、注册、改自己密码），也可能刚补完 changes。
		actor := d.actor
...
// SetActor 覆盖本条请求的操作者（登录/注册/改自己密码这类"操作的瞬间才知道是谁"的端点）。
...
// SetAction 覆盖本条请求的动作码：同一条路由因请求体分流到不同语义时用它
// （例如 PUT /admin/users/:id/ban 的封禁与解封、POST /auth/login 的成功与失败）。
func SetAction(c *gin.Context, action string) {
	if d := currentDraft(c); d != nil && strings.TrimSpace(action) != "" {
		d.entry.Action = strings.TrimSpace(action)
	}
}
...
// FailError 从 handler 层的统一 respond() 里调用：把业务错误码原样落进审计行。
func FailError(c *gin.Context, err error) {
	if err == nil {
		return
	}
	Fail(c, firstCode(err.Error()))
}
